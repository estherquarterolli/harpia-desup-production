...
from apps.professors.models import ContractType
...
def seed_paracambi():
    ...
    for row in load_paracambi_professores():
        contrato, _ = ContractType.objects.update_or_create(
            nome=row["contract_name"],
            defaults=row["contract_defaults"],
        )
        update_or_create_professor(
            id_funcional=row["id_funcional"],
            rh_matricula=row["rh_matricula"],
            defaults={
                "rh_nome": row["rh_nome"],
    ...
    curso_tga_global, _ = Course.objects.get_or_create(
        sigla="TGA",
        defaults={"nome": "Tecnologia em Gestão Ambiental"},
    )
    curso_ads_global, _ = Course.objects.get_or_create(
        sigla="ADS",
    ...
    for periodo, cod, nome, cred, ch in tga_disciplinas:
        cc, _ = CurricularComponent.objects.get_or_create(
            nome=nome,
            defaults={"carga_horaria_padrao": ch},
        )
